...
======================
    ...
    **详述**
    时间被储存在一个特别的时区中。然而，这个时区可能不会显示在界面中。未标注的时间值以UTC格式被指定、储存，和查看。
    ...
    在Studio中的设定，被标注为一个时区，例如课程开始和结束的时间，输入设定在时区中具体指明出来 (通常是您当前的时区)。
	
    未标注时区的时间设定，例如课程内容的发行日期和截止日期，从您的当地时区进行转换，并以UTC格式设定日期和时间。您可以使用线上时区转换来转换成为您的当地时区时间。
	
    ~~~*请注意当您使用线上转换时，需要考虑夏令时可能带来的影响。*
    ...
    *请注意一般不建议设定这样的延长期。它可能导致问题在应该关闭时没能关闭，同时也可能会误导您的学生。*
    ...
    如果您有进一步关于指定时间和时区的问题，或者发现了截止日期或更新日期不一致的情况，请从edX Studio的帮助页面联系我们。
